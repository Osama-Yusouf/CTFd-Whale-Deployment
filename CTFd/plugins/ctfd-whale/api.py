...
class AdminContainers(Resource):
    ...
    def patch():
        ...
        result, message = ControlUtil.try_renew_container(user_id=int(user_id), container_id=container_id)
        if not result:
    ...
    def delete():
        ...
        container_id = request.args.get('container_id')
        result, message = ControlUtil.try_remove_container(user_id, container_id=container_id)
...
class UserContainers(Resource):
    ...
    def delete():
        user_id = current_user.get_current_user().id
        challenge_id = request.args.get('challenge_id')
        result, message = ControlUtil.try_remove_container(user_id, challenge_id=challenge_id)
        if not result:
            abort(403, message, success=False)
        return {'success': True, 'message': message}
