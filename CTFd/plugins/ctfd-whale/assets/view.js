...
function loadInfo() {
    ...
    CTFd.fetch(url, {
        ...
        if (response.remaining_time != undefined) {
            ...
            let initial_seconds = parseInt(response.remaining_time);
            $('#whale-challenge-count-down').data('seconds', initial_seconds);
            $('#whale-challenge-count-down').text(Math.floor(initial_seconds / 60) + 'm ' + (initial_seconds % 60) + 's');
            
            $('#whale-panel-stopped').hide();
            $('#whale-panel-started').show();

            window.t = setInterval(() => {
                const c = $('#whale-challenge-count-down').data('seconds');
                if (c === undefined) return;
                let second = parseInt(c) - 1;
                if (second <= 0) {
                    loadInfo();
                }
                $('#whale-challenge-count-down').data('seconds', second);
                $('#whale-challenge-count-down').text(Math.floor(second / 60) + 'm ' + (second % 60) + 's');
            }, 1000);
        } else {
